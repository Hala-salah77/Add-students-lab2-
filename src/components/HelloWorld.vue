<template>
    <div class="container">
        ...
        <div class="row my-5 align-items-stretch" v-if="!isCartVisible && !isListVisible">
            <div class="col-md-3 my-3 align-items-stretch d-flex" v-for="book in booksData" :key="book.ISBN">
        ...
        <div class="row" v-if="isCartVisible">
            <div class=" col-12 align-items-center mt-5">
                ...
                <table class="table table-striped text-center">
                    ...
                    <tbody>
                        <tr v-for="book in cart.books" :key="book.book.ISBN">
                            <th scope="row">{{book.book.ISBN}}</th>
                            <td>{{book.book.title}}</td>
                            <td>{{book.book.price.value}}</td>
                            <td>{{book.quantity}}</td>
                            <td>
                                <botton class="btn bg-primary mx-2 px-3 py-1 text-white fw-bold"
                                    @click="increaseQuantity(book)">+</botton>
                                <botton class="btn bg-danger px-3 py-1 text-white fw-bold"
                                    @click="decreaseQuantity(book)">-</botton>
                            </td>
        ...
        <div class="row" v-if="isListVisible">
            <div class=" col-12 align-items-center mt-5">
                ...
                <table class="table table-striped text-center">
                    ...
                    <tbody>
                        <tr v-for="book in list.books" :key="book.book.ISBN">
                            <th scope="row">{{book.book.ISBN}}</th>
                            <td>{{book.book.title}}</td>
                            <td>{{book.book.price.value}}</td>
                            ...
                            <td>
                                <botton class="btn bg-danger px-3 py-1 text-white fw-bold"
                                    @click="removeFromList(book.book)">Remove From List</botton>
                            </td>
                        </tr>
                    </tbody>
                </table>
